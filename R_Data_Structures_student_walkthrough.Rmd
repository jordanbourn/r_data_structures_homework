...
# vector1 is repeated to make (1,2,3,1,2,3)
```
...
student_class_level <- c('Sophomore','Junior','Senior')
student_class_grade <- c(65, 82,91)

# Create a dataframe from these three vectors, each vector is it's own column
df <- data.frame(student_name, student_class_level, student_class_grade)
df
```
...
```{r}
student_grades <- df$student_class_level
student_grades
```
...
student_class_level <- c('Freshman','Senior')
...
matrix1 <- matrix(first_vector, dim <- c(3,3))
matrix2 <- matrix(second_vector, dim <- c(3,3))

mat_array <- array(c(matrix1,matrix2),dim <- c(3,3,2),dimnames <- list(c('ROW 1','ROW 2','ROW 3'),c('COL 1','COL 2','COL 3'),c('Matrix 1','Matrix 2')))
mat_array
```
